...
Qwen-Agent 是一个**本地运行的编码 agent**:它接一个 OpenAI 兼容的本地推理后端(作者用 [oMLX](https://github.com/jundot/omlx) 在 Apple Silicon 上跑 Qwen3.6-35B-A3B),用 6 个基础工具(+ 2 个 codegraph 智能定位工具,有索引时自动启用)端到端完成编码任务——自己探索代码、定位、修改、验证。
...
Qwen-Agent 省的是**执行**的 token。但 Claude 出规格 / 验收前要**读懂代码**(定位符号、查调用关系),用 grep + read 一堆文件本身也烧 token。**codegraph**(代码索引)把理解端也省了:预建索引 sub-ms 查询,返回精确 `file:line` + 源码,不用读整文件就定位。
